      ...
      document
        ...
        .addEventListener("submit", function (e) {
          ...
          const whatsappURL = `https://wa.me/212712825177?text=${encodeURIComponent(
            message
          )}`;
...
الرسالة: ${message}
      ...
      // إدارة القائمة السفلية
      document.querySelectorAll(".bottom-nav-item").forEach((item) => {
        item.addEventListener("click", function (e) {
          if (this.id === "installBtn") {
            e.preventDefault();
            promptInstall();
            return;
          }
      ...
      function promptInstall() {
        if (!deferredPrompt) {
          alert(
            'لتثبيت التطبيق، استخدم خلية "إضافة إلى الشاشة الرئيسية" في متصفحك'
          );
          return;
        }
